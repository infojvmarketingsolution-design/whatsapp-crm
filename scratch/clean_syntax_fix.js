...
// Use a very robust regex to find the start of the showProfile block 
// and replace everything until the start of the ADVANCED FILTER block.

const startRegex = /\{showProfile && selectedContact && editedContact && \(/;
const filterHeader = '/* ADVANCED FILTER PRO CONSOLE (SIDEBAR) */';

const startIndex = content.search(startRegex);
const filterIndex = content.indexOf(filterHeader);

if (startIndex !== -1 && filterIndex !== -1) {
    const head = content.substring(0, startIndex);
    const tail = content.substring(filterIndex);
    
    const newProfile = `{showProfile && selectedContact && editedContact && (
    ...
    fs.writeFileSync(filePath, head + newProfile + tail);
    console.log('Profile cleaned and syntax error fixed.');
} else {
    console.log('Could not find anchor points for replacement.');
}
